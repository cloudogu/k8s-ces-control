...
import (
	...
	"github.com/cloudogu/k8s-ces-control/generated/dummy"
...
func Test_authorize(t *testing.T) {
	emptyGetCredentialsFunc := func(_ context.Context, serviceName string) (account.ServiceAccountData, error) {
		return account.ServiceAccountData{}, nil
	}

	correctGetCredentialsFunc := func(_ context.Context, serviceName string) (account.ServiceAccountData, error) {
		return account.ServiceAccountData{Username: "admin", Password: "password"}, nil
	}

	errorGetCredentialsFunc := func(_ context.Context, serviceName string) (account.ServiceAccountData, error) {
		return account.ServiceAccountData{}, assert.AnError
	}

	wrongGetCredentialsFunc := func(_ context.Context, serviceName string) (account.ServiceAccountData, error) {
	...
	t.Run("should return error as no servicename header is present", func(t *testing.T) {
		md := metadata.MD{}
		err := authorize(context.TODO(), &md, "/doguAdministration.DoguAdministration/GetDoguList", emptyGetCredentialsFunc)
		require.Error(t, err)
	...
	t.Run("should return error as serviceAccountData could not be accessed", func(t *testing.T) {
		...
		err := authorize(context.TODO(), &md, "/doguAdministration.DoguAdministration/GetDoguList", errorGetCredentialsFunc)
		require.Error(t, err)
	...
	t.Run("should return error as username does not match", func(t *testing.T) {
		...
		md.Set("servicename", "test")
		md.Set("authorization", "user:password")
		err := authorize(context.TODO(), &md, "/doguAdministration.DoguAdministration/GetDoguList", wrongGetCredentialsFunc)
		require.Error(t, err)
	...
	t.Run("should return no error as auth header and credentials match", func(t *testing.T) {
		...
		err := authorize(context.TODO(), &md, "/doguAdministration.DoguAdministration/GetDoguList", correctGetCredentialsFunc)
...
func getGrpcClient(t *testing.T) (dummy.HelloWorldServiceClient, *context.Context, *grpc.ClientConn) {
	...
	conn, clientErr := grpc.DialContext(ctx, "bufnet", grpc.WithContextDialer(bufDialer), grpc.WithTransportCredentials(insecure.NewCredentials()))
	if clientErr != nil {
