...
func Test_doguLogLevelRegistry_UnMarshalFromStringToCesRegistry(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		doguConfigRepositoryMock.EXPECT().Get(context.TODO(), common.SimpleName("dogua")).Return(doguConfigA, nil)
		doguConfigRepositoryMock.EXPECT().Get(context.TODO(), common.SimpleName("dogub")).Return(doguConfigB, nil)
