...
func Test_defaultDoguInterActor_SetLogLevelInAllDogus(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		doguConfigRepositoryMock.EXPECT().Get(context.TODO(), dogu.SimpleName("postgresql")).Return(doguConfigA, nil)
		doguConfigRepositoryMock.EXPECT().Get(context.TODO(), dogu.SimpleName("redmine")).Return(doguConfigB, nil)
